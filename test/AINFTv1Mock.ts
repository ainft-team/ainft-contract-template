...
describe('AINFTv1Mock', function () {
  ...
  describe('Deployment', function () {
    ...
    it('Should support AINFTv1 interface', async function () {
      ...
      expect(await ainftV1Mock.supportsInterface(0x9de93351)).to.equal(true); // AINFTBaseV1
    });
  });
  ...
  describe('Mint', function () {
    describe('Validations', function () {
      ...
      it("Should revert with the right error if invalid 'to' address given", async function () {
        ...
        await expect(ainftV1Mock.connect(minter).mint(ethers.constants.AddressZero, 1)).to.be.revertedWith(
      ...
      it('Should revert with the right error if exceeds maxTokenId', async function () {
        ...
        await expect(ainftV1Mock.connect(minter).mint(users[0].address, maxTokenId)).not.to.be.reverted;
  ...
  describe('Tokens Of', function () {
    it('Should fail if invalid limit is given', async () => {
      ...
      await expect(ainftV1Mock.tokensOf(users[0].address, 0, 0))
        .to.be.revertedWithCustomError(ainftV1Mock, 'InvalidLimit')
        .withArgs(0, 100);
    ...
    it('Should fail if invalid offset is given', async () => {
      ...
      await expect(ainftV1Mock.tokensOf(users[0].address, 2, 1))
        .to.be.revertedWithCustomError(ainftV1Mock, 'InvalidOffset')
        .withArgs(2, 1);
    });
  ...
  describe('Burn', function () {
    it('Should fail with a nonexistent token ID', async () => {
      ...
      await expect(ainftV1Mock.connect(owner).burn(1)).to.be.revertedWith('ERC721: invalid token ID');
    });
  ...
  describe('Set maxTokenId', function () {
    ...
    it('Should fail with an invalid value', async () => {
      ...
      await expect(ainftV1Mock.connect(owner).setMaxTokenId(nextTokenId - 2)).to.be.revertedWith(
  ...
  describe('Destroy', function () {
    ...
    it('Should fail with an invalid payable address', async () => {
      ...
      await expect(ainftV1Mock.connect(owner).destroy(ethers.constants.AddressZero)).to.be.revertedWith(
        'AINFTv1: invalid to_ address',
      );
    });
